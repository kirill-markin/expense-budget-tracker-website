---
title: מסמך עזר ל-API
description: תהליך חיבור סוכנים ומסמך עזר ל-SQL API לצורך גישה תכנותית לנתונים הפיננסיים שלכם.
---

## סקירה כללית

Expense Budget Tracker חושפת API ציבורי אחד לגישה תכנותית בכתובת:
...
כל הבקשות כפופות לאותה אכיפת Postgres Row Level Security שבה משתמשת אפליקציית הווב.
...
תגובת הגילוי מסבירה לסוכנים איך לאתחל את תהליך האימות ולאילו נקודות קצה לפנות בהמשך. אותו API גם מפרסם:
...
- `X-Workspace-Id: <workspaceId>` עדיין נתמך אם תרצו לעקוף את סביבת העבודה השמורה עבור בקשה אחת
...
סקריפטים, משימות cron, לוחות מחוונים ואפליקציות מותאמות אישית יכולים לפנות לאותו API ישירות, כל עוד כבר יש להם `ApiKey` ארוך-טווח.
...
`X-Workspace-Id` נדרש רק אם למפתח עדיין אין סביבת עבודה שמורה כברירת מחדל, או אם תרצו לעקוף את סביבת העבודה השמורה עבור אותה בקשה.
...
- `GET /v1/me` — פרטי החשבון לאחר אימות
...
`POST /v1/sql` מקבל בדיוק משפט SQL אחד בכל בקשה.
...
- הערות SQL
- מזהים במירכאות
- מחרוזות בתבנית dollar-quoted

השרת גם מגביל אילו טבלאות אפשר לתשאל. השתמשו ב-`/v1/schema` כדי לבדוק אילו טבלאות ועמודות נחשפות לפני יצירת SQL.
