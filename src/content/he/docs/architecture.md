...
לקוחות תוכנה --> API Gateway (REST) --> SQL Lambda -------^
לקוחות MCP   --> API Gateway (HTTP) --> MCP Lambda -------^
שירות אימות (OTP + OAuth) --------------------------------^
תהליך רקע (איסוף שערי חליפין) ----------------------------^
```

שישה רכיבים, מסד נתונים אחד:
...
4. **mcp** — פונקציית AWS Lambda ו-HTTP API ייעודיים ב-API Gateway עבור נקודת הקצה המתארחת של MCP
5. **worker** — מושך מדי יום שערי חליפין מ-ECB, CBR ו-NBS
6. **Postgres** — מקור האמת היחיד, עם אבטחה ברמת שורה
...
לקוחות MCP מתחברים בנפרד אל `https://mcp.expense-budget-tracker.com/mcp`. HTTP API ייעודי ב-API Gateway ופונקציית MCP Lambda מטפלים במסלול הזה, ואילו ההרשאה האינטראקטיבית מתבצעת באמצעות OAuth בדפדפן בדומיין האימות.
