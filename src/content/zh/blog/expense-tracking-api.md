...
公开入口是 [Expense Budget Tracker 发现文档](https://api.expense-budget-tracker.com/v1/)。它会说明当前的身份验证流程，引导客户端查看运行时数据库结构，链接到开源实现，并告诉脚本或智能体下一步该调用哪个接口。
...
[API 参考](/zh/docs/api/)适合查询端点细节，[智能体接入指南](/zh/docs/agent-setup/)则会逐步介绍邮箱验证码流程。如果这些文档与实时发现响应或 `/v1/schema` 不一致，请遵循实时接口契约。
...
发现文档说明接入和传输流程；需要身份验证的 `/v1/schema` 则说明当前所选工作区向这把密钥开放了哪些数据库内容。
...
实时 SQL 契约允许 `/v1/sql` 接收一条或多条以分号分隔的 `SELECT`、`WITH`、`INSERT`、`UPDATE` 或 `DELETE` 语句。多语句支持很实用，但不代表应该把整次导入塞进一个难以检查的请求里。
...
每次构建或更新可复用客户端时，都应重新从当前发现说明、其中链接的源代码和 `/v1/schema` 读取这些规则。它们是接口契约的一部分，不是可以忽略的实现细节。
