---
title: AI 智能体接入设置
description: 把同一个发现入口 URL 提供给 Claude Code、Codex 或 OpenClaw。智能体会读取发现响应、验证 8 位验证码、保存长期有效的 ApiKey，然后开始处理你的工作区任务。
---
...
这个端点是面向 AI 智能体的标准公开发现文档。它会告诉智能体如何初始化认证、下一步该调用哪些端点、后续应使用哪种认证方式，以及到哪里获取已发布的 API 规范。
...
4. 检查收件箱，获取 8 位验证码。
5. 把验证码发回给智能体。
6. 让智能体把返回的 ApiKey 保存在聊天记忆之外，然后继续执行导入、查询或预算相关任务。
...
2. 读取发现响应，并按照返回的 action 继续，而不是把后续步骤写死
3. 将用户邮箱 `POST` 到返回的 `bootstrapUrl`
4. 收到 `otpSessionToken` 和 `verify_code` action
5. 向用户请求 8 位邮箱验证码
...
工作区选择是显式操作，但不是无状态的。调用 `POST /v1/workspaces/{workspaceId}/select` 之后，所选工作区会为该 `ApiKey` 保存下来，因此后续调用 `/v1/sql` 时可以省略 `X-Workspace-Id`。如果你想在某一次请求中覆盖已保存的工作区，仍然可以显式发送 `X-Workspace-Id`。

如果用户恰好只有一个工作区，并且这个 `ApiKey` 还没有保存过默认选择，后端会自动保存并使用该工作区。

## 智能体应该保存什么

`verify-code` 返回的 `ApiKey` 是长期有效的。不要只依赖聊天记忆。
...
然后按照服务器返回的 action 继续。只要发现文档已经提供了后续步骤，就不要自行假设或写死流程。

在 `verify-code` 之后，请安全保存返回的 `ApiKey`。然后继续读取：
...
在需要时使用我的账户邮箱，向我询问 8 位验证码，把返回的 ApiKey 保存在聊天记忆之外，然后从 ~/Downloads 导入我最新的银行对账单。
```
...
先询问我的邮箱，等待邮件验证码，保存返回的 ApiKey，检查 /schema，然后把我最新的交易记录展示出来。
```
...
登录完成后，列出我的工作区，把其中一个保存为这个 ApiKey 的默认工作区，然后导入我刚上传的新 CSV。
```
