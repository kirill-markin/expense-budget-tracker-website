...
1. **web**：提供界面仪表盘和 API 路由的 Next.js 应用
2. **auth**：运行在认证域名上的邮箱 OTP 登录与 Agent 接入服务
...
- **ledger_entries**：每条账户资金变动一行记录，包括收入、支出和转账
- **budget_lines**：仅追加的预算单元格记录，以最后一次写入为准
...
- **accounts**：由 ledger entries 派生出的视图

## 多币种

所有金额都以原生货币存储。转换为报表货币时，会在读取阶段通过 SQL 与 `exchange_rates` 做关联。不会进行有损的预转换。
...
对于机器客户端，公开的发现入口是 `GET /v1/`。Agent 接入流程在认证域名上通过邮箱 OTP 完成，返回长期有效的 ApiKey，随后通过 API Gateway 提供的机器 API 执行 SQL。
