...
Машинні клієнти      --> API Gateway (REST) --> SQL Lambda ---------^
MCP-клієнти          --> API Gateway (HTTP) --> MCP Lambda ---------^
Сервіс автентифікації (OTP + OAuth) -------------------------------^
Worker (оновлення курсів FX) --------------------------------------^
```

Архітектура складається із шести компонентів, які працюють з однією базою даних:
...
4. **mcp** — окремі AWS Lambda й HTTP API в API Gateway для розміщеної точки MCP
5. **worker** — процес, що щодня завантажує валютні курси з ECB, CBR і NBS
6. **Postgres** — єдине джерело істини з безпекою на рівні рядків
...
MCP-клієнти підключаються окремо до `https://mcp.expense-budget-tracker.com/mcp`. Цей шлях обслуговують виділені HTTP API в API Gateway і MCP Lambda, а інтерактивна авторизація відбувається через OAuth у браузері на домені автентифікації.
