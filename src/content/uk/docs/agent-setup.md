...
Це канонічна публічна точка входу для AI-агентів. Вона пояснює агенту, як розпочати автентифікацію, які ендпоїнти викликати далі, який спосіб автентифікації використовувати після входу і де знайти runtime-схему та відкритий вихідний код реалізації.

Цей посібник описує пряме підключення через Agent API для термінальних агентів і HTTP-клієнтів. Якщо ваш клієнт підтримує MCP, скористайтеся розміщеним [MCP-конектором](/uk/docs/mcp-connector/): він підключається до `https://mcp.expense-budget-tracker.com/mcp` та інтерактивно авторизується через OAuth у браузері без цього процесу отримання `ApiKey`.
...
14. Читати дані через `POST https://api.expense-budget-tracker.com/v1/sql/query`, а схвалені зміни надсилати через `POST https://api.expense-budget-tracker.com/v1/sql/execute`

Вибір робочого простору задається явно, але не скидається після кожного запиту. Після `POST /v1/workspaces/{workspaceId}/select` вибраний робочий простір зберігається для цього API-ключа, тому в наступних викликах `/v1/sql/query` і `/v1/sql/execute` можна не передавати `X-Workspace-Id`. За потреби ви все одно можете надіслати `X-Workspace-Id`, щоб тимчасово перевизначити збережений робочий простір для конкретного запиту. Сумісний маршрут `POST /v1/sql` призначений лише для атомарних сценаріїв із кількох інструкцій, а не для звичайного читання чи запису.
...
- `/v1/openapi.json` або `/v1/swagger.json` лише якщо клієнт перевіряє стандартні URL специфікації: ці адреси повертають посилання на discovery і вихідний код, а не специфікацію API
