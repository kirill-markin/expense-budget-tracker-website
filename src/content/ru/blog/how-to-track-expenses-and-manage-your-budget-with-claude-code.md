...
description: "Настройте Claude Code как помощника по личным финансам: передайте ему discovery URL, войдите по коду из письма, сохраните полученный ApiKey, и он сможет разбирать выписки, сверять балансы и помогать с бюджетом прямо из терминала."
date: "2026-03-05"
keywords:
...
[Expense Budget Tracker](https://expense-budget-tracker.com/ru/) - это система личных финансов с открытым исходным кодом на базе Postgres. Основной discovery URL сервиса: `GET https://api.expense-budget-tracker.com/v1/`. Можно зарегистрироваться в облачной версии или [развернуть систему самостоятельно](https://github.com/kirill-markin/expense-budget-tracker) на своем сервере.
...
Сначала Claude Code должен прочитать ответ по discovery URL, а затем запросить:
...
Сама процедура входа удобная, но ключ все равно нужно сохранить в надежном месте. Бэкенд прямо предупреждает агентов, что нельзя полагаться только на историю диалога.
...
Claude Code работает заметно лучше, если заранее описать ему свои категории, счета и рабочие правила. Для этого удобно завести локальный файл `CLAUDE.md`:
...
Claude Code запросит представление `accounts` через SQL API, сравнит остатки и покажет расхождения. Если по `chase-checking` получилось $4,180.15 вместо $4,230.15, агент поможет найти пропавшие $50: возможно, одна транзакция не импортировалась или, наоборот, была учтена дважды.
...
Claude Code сам напишет SQL, выполнит его через API и объяснит результат обычным языком. Самому знать SQL не обязательно, но при желании можно попросить показать запрос, проверить его и уточнить формулировку.
...
API для машинного доступа в Expense Budget Tracker открывает небольшой набор отношений, с которыми ИИ-агенту удобно работать. Список разрешенных отношений публикуется через `GET /v1/schema`.
...
API-ключи хранятся как SHA-256-хэши, открытый текст никогда не лежит в базе. Ключ можно позже отозвать из продукта. Лимиты ограничивают использование: 10 запросов в секунду, 10 000 запросов в день, таймаут 30 секунд и максимум 100 строк в одном ответе.
...
Если у вас уже есть долгоживущий ApiKey Expense Budget Tracker, Claude Code может пропустить вход по коду из письма и сразу использовать существующий ключ. В этом режиме он обращается к тем же маршрутам API:
...
Такой вариант удобен для постоянных скриптов и заранее подготовленных окружений, но для большинства пользователей discovery URL и вход по одноразовому коду остаются самым простым способом настройки.
