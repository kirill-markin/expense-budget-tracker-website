---
title: "كيف تتتبّع المصروفات وتدير ميزانيتك باستخدام Claude Code"
description: "اضبط Claude Code ليعمل كمساعدك المالي الشخصي. أعطه رابط اكتشاف واحدًا، ودعه يُكمل تسجيل الدخول برمز التحقق المرسل إلى البريد، ثم احفظ ApiKey الذي يعيده ليتمكن من تحليل الكشوف، والتحقق من الأرصدة، وإدارة ميزانيتك من الطرفية."
date: "2026-03-05"
---

Claude Code هو وكيل ذكاء اصطناعي من Anthropic يعمل من داخل الطرفية. يمكنه قراءة الملفات وكتابة الشيفرة وتنفيذ الأوامر وإرسال طلبات HTTP. يستخدمه معظم الناس في تطوير البرمجيات، لكنه مناسب أيضًا جدًا للتمويل الشخصي عندما توصله بمتتبّع مصروفات يوفّر واجهة برمجة تطبيقات مخصصة للوكلاء وواضحة البنية.

الفكرة بسيطة: صِل Claude Code بمتتبّع مصروفات مفتوح المصدر عبر واجهته البرمجية، فيتحول إلى مساعد مالي شخصي يعمل من الطرفية. أسقط كشفًا بنكيًا في مجلدك، واطلب منه تسجيل المعاملات، والتحقق من الأرصدة، وتحديث الميزانية، وكل ذلك من خلال محادثة طبيعية. لا حاجة للتنقل بين الشاشات ولا لإدخال البيانات يدويًا.
...
**يعمل محليًا ويقرأ ملفاتك مباشرة.** عندما تنزّل كشفًا بنكيًا بصيغة CSV أو PDF، يستطيع Claude Code قراءته مباشرة من نظام الملفات لديك. لا حاجة إلى رفع الملف أو نسخه ولصقه أو التقاط صور للشاشة. يكفي أن تقول: "حلّل الكشف البنكي الموجود في `~/Downloads/chase-march-2026.csv`" ليقرأ الملف بنفسه.

**ينفّذ الشيفرة ويرسل طلبات HTTP بنفسه.** Claude Code لا يكتفي باقتراح أمر `curl`، بل ينفّذه. وإذا احتاج إلى إدخال 50 معاملة في قاعدة بيانات المصروفات، فإنه يكتب SQL، ويرسل طلب HTTP، ثم يتأكد من النتيجة. كل هذا يحدث داخل المحادثة نفسها.

**يتذكر إعدادك بين الجلسات.** بعد حفظ `ApiKey` الذي تعيده الخدمة خارج ذاكرة الدردشة، يستطيع Claude Code إعادة استخدام الاتصال نفسه في الجلسات اللاحقة بدل تكرار تسجيل الدخول عبر رمز البريد في كل مرة.

**يعالج الملفات محليًا قبل أن يلمس الـ API.** إذا أردت تنظيف ملف CSV أو تهيئة كشوف بنكية أو كتابة سكربت استيراد، فيمكنه إنجاز ذلك محليًا أولًا قبل إرسال أي شيء إلى الواجهة البرمجية.

## إعداد Claude Code للتمويل الشخصي

أنت تحتاج إلى شيئين: متتبّع مصروفات يوفّر واجهة برمجة تطبيقات مخصصة للوكلاء، ومكان تحفظ فيه المفتاح طويل الأجل الذي يحصل عليه Claude Code بعد تسجيل الدخول.

[Expense Budget Tracker](https://expense-budget-tracker.com/ar/) هو نظام مفتوح المصدر للتمويل الشخصي مبني على Postgres. ورابط الاكتشاف الأساسي له هو `GET https://api.expense-budget-tracker.com/v1/`. يمكنك إنشاء حساب في النسخة المستضافة أو [استضافته ذاتيًا](https://github.com/kirill-markin/expense-budget-tracker) على خادمك.
...
المفترض أن يبدأ بقراءة استجابة الاكتشاف، ثم يطلب منك:

- بريدك الإلكتروني المرتبط بالحساب
- الرمز المكوّن من 8 أرقام الذي وصل إلى صندوق الوارد

بعد التحقق من الرمز، تعيد الخدمة مفتاحًا طويل الأجل بصيغة API الفعلية، مثل `ebta_...`.

### الخطوة 2: احفظ المفتاح المُعاد خارج ذاكرة الدردشة

تسجيل الدخول بهذه الطريقة مريح، لكن المفتاح ما يزال يحتاج إلى مكان حفظ دائم. والخلفية نفسها توضح صراحةً أن الوكلاء يجب ألا يعتمدوا على سجل الدردشة وحده.
...
بعد ذلك، يمكن لواجهة `/v1/sql` العمل من دون `X-Workspace-Id`. وإذا كان حسابك يحتوي على مساحة عمل واحدة فقط، فستُحفظ تلقائيًا وتُستخدم من أول مرة.

### الخطوة 4: أضف ملف تعليمات محليًا يعبّر عن طريقتك

سيكون أداء Claude Code أفضل عندما تعطيه فئاتك وحساباتك وقواعد عملك الخاصة. ولهذا يفيد وجود ملف `CLAUDE.md` محلي:
...
تتبّع المصروفات يعني تسجيل ما حدث فعلًا. أما إعداد الميزانية فيعني التخطيط لما سيأتي بعد ذلك. وكلاهما يعيش في قاعدة البيانات نفسها.
...
**المعالجة الدفعية.** ضع خمسة كشوف بنكية من حسابات مختلفة في مجلد واحد، واطلب من Claude Code معالجتها كلها. سيقرأ كل ملف، ويدخل المعاملات في الحساب المناسب، ثم يتحقق من الأرصدة في النهاية. تنفيذ ذلك عبر واجهة ويب قد يعني ساعة كاملة من النقر.

**التحليل المخصص.** "ما الأشهر التي شهدت أعلى إنفاق على الترفيه في السنة الماضية، وما أكبر المعاملات فيها؟" لا يوجد زر جاهز لهذا في تطبيقات الميزانية. Claude Code يكتب استعلام SQL، ويشغّله، ثم يشرح النتيجة.
...
تُخزَّن مفاتيح الـ API على هيئة SHA-256 hashes، ولا تُحفظ قيمها النصية الصريحة داخل قاعدة البيانات. ويمكنك إلغاء المفاتيح لاحقًا من داخل المنتج. كما أن حدود المعدل تقيد الاستخدام عند 10 طلبات في الثانية و10,000 طلب يوميًا، مع مهلة قدرها 30 ثانية وحد أقصى 100 صف في كل استجابة.

يبقى مفتاح الـ API في متغير البيئة المحلي لديك. ويقرأه Claude Code من `$EXPENSE_BUDGET_TRACKER_API_KEY` عند إرسال الطلبات، ولا يحتاج أبدًا إلى حفظه داخل المشروع.

## بديل متقدم: HTTP مباشر من دون تسجيل دخول مخصص للوكلاء

إذا كان لديك أصلًا `ApiKey` طويل الأجل لـ Expense Budget Tracker، فيستطيع Claude Code تجاوز إعداد رمز التحقق المرسل إلى البريد واستخدام المفتاح الموجود مباشرة. وفي هذا الوضع ستتعامل مع نقاط النهاية نفسها:

- `GET /v1/openapi.json` للحصول على المواصفة المنشورة القابلة للقراءة آليًا
- `GET /v1/schema` للحصول على العلاقات المسموح بها
- `POST /v1/sql` لتنفيذ الاستعلامات الفعلية

هذا الأسلوب مفيد للسكربتات الثابتة والبيئات المجهزة مسبقًا، لكن بالنسبة لمعظم الناس يبقى رابط الاكتشاف مع تدفق رمز البريد هو الإعداد الأسهل.

## سير عمل واقعي: تتبّع أسبوعي للمصروفات في 10 دقائق

يعتمد Kirill Markin هذا الأسلوب نفسه منذ سنوات، وغالبًا ما تنتهي الجلسة الأسبوعية إلى شيء قريب من الآتي:

1. تنزيل الكشوف البنكية من جميع الحسابات خلال دقيقتين
2. فتح Claude Code وطلب معالجة الملفات خلال 3 دقائق، بينما يقوم هو بالعمل وأنت تراجع
3. مراجعة ما سجّله Claude Code وتصحيح أي فئات خاطئة خلال 3 دقائق
4. طلب التحقق من تطابق أرصدة الحسابات مع البنك خلال دقيقة واحدة
5. وإذا كان هذا آخر الشهر، طلب مقارنة القيم الفعلية بالميزانية وتحديث التوقّع خلال دقيقتين

أي إنك تحتاج إلى 10 دقائق فقط لتحصل على صورة مكتملة عن وضعك المالي: كل معاملة مصنفة، وكل رصيد متحقق منه، والميزانية محدّثة. وينجح هذا الأسلوب لأن الأجزاء المملة، مثل التحليل والتصنيف والإدخال والحساب، هي بالضبط ما يجيده Claude Code، بينما تبقى أجزاء الحكم البشري، مثل مراجعة الفئات واتخاذ قرارات تعديل الميزانية، لديك أنت.

## بدء الاستخدام مع Claude Code وExpense Budget Tracker

1. [ثبّت Claude Code](https://docs.anthropic.com/en/docs/claude-code) إذا لم تكن قد فعلت ذلك بعد
2. أنشئ حسابًا على [expense-budget-tracker.com](https://expense-budget-tracker.com/ar/) أو [استضف التطبيق ذاتيًا](https://github.com/kirill-markin/expense-budget-tracker)
3. أعطِ Claude Code الرابط `https://api.expense-budget-tracker.com/v1/`
4. أكمل تسجيل الدخول برمز البريد، ثم احفظ المفتاح المُعاد في `EXPENSE_BUDGET_TRACKER_API_KEY`
5. احفظ مساحة عمل افتراضية لهذا المفتاح
6. أضف ملف `CLAUDE.md` محليًا يتضمن فئاتك وحساباتك وقواعد عملك
7. افتح Claude Code داخل مجلدك المالي، ثم أضف أول كشف بنكي

سيفحص Claude Code المخطط، ويطابق فئاتك، ويبدأ تسجيل المعاملات. راجع النتائج، وصحّح أي شيء يبدو غير دقيق، وستحصل على إعداد ذكي لتتبّع المصروفات يعمل مباشرة من الطرفية.

متتبّع المصروفات مرخّص بترخيص MIT ومفتوح المصدر بالكامل على [github.com/kirill-markin/expense-budget-tracker](https://github.com/kirill-markin/expense-budget-tracker). أما Claude Code فمتاح على [docs.anthropic.com/en/docs/claude-code](https://docs.anthropic.com/en/docs/claude-code). ويمكن البدء بالأداتين مجانًا.
