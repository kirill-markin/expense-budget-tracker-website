...
کلاینت‌های ماشینی    --> API Gateway (REST) --> SQL Lambda -----^
کلاینت‌های MCP       --> API Gateway (HTTP) --> MCP Lambda -----^
سرویس احراز هویت (OTP + OAuth) -------------------------------^
Worker (گردآورنده‌های نرخ ارز) -------------------------------^
```

شش جزء، یک پایگاه داده:
...
4. **mcp** — AWS Lambda و HTTP API اختصاصی در API Gateway برای endpoint میزبانی‌شدهٔ MCP
5. **worker** — واکشی روزانهٔ نرخ ارز از ECB، CBR و NBS
6. **Postgres** — پایگاه دادهٔ واحد و مرجع نهایی داده‌ها با RLS
...
کلاینت‌های MCP جداگانه به `https://mcp.expense-budget-tracker.com/mcp` متصل می‌شوند. HTTP API اختصاصی در API Gateway و MCP Lambda این مسیر را مدیریت می‌کنند و مجوز تعاملی با OAuth در مرورگر روی دامنهٔ احراز هویت انجام می‌شود.
