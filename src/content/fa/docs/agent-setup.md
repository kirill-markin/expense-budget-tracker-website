...
این نشانی، سند کشف عمومی و مرجع اصلی برای عامل‌های هوش مصنوعی است. این سند به عامل می‌گوید فرایند احراز هویت را چطور شروع کند، بعد سراغ کدام مسیرها برود، از چه روش احراز هویتی استفاده کند و شِمای زمان اجرا و پیاده‌سازی متن‌باز را از کجا پیدا کند.

این راهنما اتصال مستقیم از طریق Agent API را برای عامل‌های ترمینال و کلاینت‌های HTTP توضیح می‌دهد. اگر کلاینت شما از MCP پشتیبانی می‌کند، به‌جای آن از [اتصال‌دهندهٔ MCP](/fa/docs/mcp-connector/) میزبانی‌شده استفاده کنید؛ این اتصال‌دهنده به `https://mcp.expense-budget-tracker.com/mcp` وصل می‌شود و بدون فرایند دریافت `ApiKey` این صفحه، به‌صورت تعاملی با OAuth در مرورگر مجوز می‌گیرد.
...
14. داده‌ها را از طریق `POST https://api.expense-budget-tracker.com/v1/sql/query` بخواند و نوشتن‌های تأییدشده را از طریق `POST https://api.expense-budget-tracker.com/v1/sql/execute` ارسال کند

انتخاب فضای کاری صریح است، اما فقط به همان درخواست محدود نمی‌ماند. بعد از `POST /v1/workspaces/{workspaceId}/select`، فضای کاری انتخاب‌شده برای همان کلید API ذخیره می‌شود؛ بنابراین در فراخوانی‌های بعدی `/v1/sql/query` و `/v1/sql/execute` لازم نیست `X-Workspace-Id` را بفرستید. با این حال، اگر بخواهید برای یک درخواست مشخص موقتاً فضای کاری ذخیره‌شده را نادیده بگیرید، همچنان می‌توانید `X-Workspace-Id` را ارسال کنید. مسیر سازگاری `POST /v1/sql` فقط برای اسکریپت‌های اتمیک چنددستوری است، نه خواندن یا نوشتن عادی.
...
- `/v1/openapi.json` یا `/v1/swagger.json` فقط وقتی کلاینت نشانی‌های متداول مشخصات را بررسی می‌کند؛ این مسیرها پیوندهای کشف و کد منبع را برمی‌گردانند، نه مشخصات API
